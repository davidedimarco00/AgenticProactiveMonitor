...
The logging pipeline collects three different streams:

- `/var/log/machine/app.log`: structured JSON application events generated by the Notes Platform;
- `/var/log/machine/system.log`: periodic system heartbeat events;
- `/var/log/machine/runtime.log`: raw stdout/stderr emitted by Gunicorn, Uvicorn, Python and the application process.

Structured application logs include `service`, `event_type`, `level`, `message` and, when available, `request_id`. HTTP responses are classified as `INFO` below 400, `WARN` for 4xx responses, and `ERROR` for 5xx responses.

Unhandled application exceptions are written as structured `ERROR` events with:
...
The stack trace is limited to 12000 characters by default and can be changed with `MAX_STACK_TRACE_CHARS`.

Runtime output is also mirrored to `runtime.log` and forwarded by Fluent Bit with `log_source=runtime`, `service=<HOST_ID>` and `event_type=runtime_output`. This provides a fallback for startup failures, framework errors and process-level messages that occur outside application exception handlers.
