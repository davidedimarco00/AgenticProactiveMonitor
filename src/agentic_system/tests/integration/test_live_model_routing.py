...
async def _exercise_live_model_routing() -> None:
    ...
    context = ContextManager(
        system_prompt=(
            "You are a monitoring specialist. Never invent live infrastructure "
            "state; use an available tool when the user explicitly requests a "
            "live verification."
        )
    )
    context.add_message_dict(
        {
            "role": "user",
            "content": (
                "Verify that the AgenticProactiveMonitor MCP server is currently "
                "available. You MUST use the available ping tool before answering."
            ),
        },
        conversation_id,
    )

    response = await provider.get_llm_response(
        context,
        tools=tools,
    ...
    assert tool_calls, (
        "Qwen did not produce a tool call after Gemma reasoning for an explicit "
        "live MCP verification request"
    )

    selected = tool_calls[0]
    selected_name = str(selected.get("name", ""))
    assert selected_name in tool_by_name, (
        f"Qwen selected a tool not present in the SPADE-LLM MCP registry: {selected_name}"
    )
    assert selected_name.endswith("_ping"), (
        f"Expected Qwen to select the MCP ping tool, got {selected_name}"
    )
    ...
    assert "ok" in str(result).lower(), (
        f"The MCP ping tool selected by Qwen did not return an OK result: {result!r}"
    )
...
@pytest.mark.skipif(
    ...
    reason="Set RUN_LIVE_MODEL_ROUTING=1 to run real Gemma/Qwen/Ollama tool routing",
)
def test_live_gemma_reasoning_qwen_selects_and_executes_real_mcp_tool() -> None:
    """Use real Ollama models and a real SPADE-LLM MCP tool end to end.

    The HybridLLMProvider calls Gemma for reasoning and Qwen for tool selection.
    The selected tool is then executed through SPADE-LLM's MCPToolAdapter, so the
    test covers the same discovery/tool abstraction used by the running agents.
    """
