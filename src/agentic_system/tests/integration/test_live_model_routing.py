...
async def _exercise_live_model_routing() -> None:
    ...
    # Keep this test focused on model routing rather than testing a large tool
    # catalogue. Full MCP discovery is covered by test_services.py.
    ...
    assert all(" " not in tool.name for tool in routing_tools)

    conversation_id = "live-model-routing"
    context = ContextManager(
        system_prompt=(
            "You are a monitoring specialist. Use a supplied tool whenever a live "
    ...
    context.add_message_dict(
        {
            "role": "user",
            "content": (
                "Verify whether the AgenticProactiveMonitor MCP server is available "
                "right now. Call the ping tool; do not answer from prior knowledge."
            ),
        },
        conversation_id,
    )
    ...
    assert tool_calls, (
        f"{provider.tool_model} did not produce a native Ollama tool call after "
        "Gemma reasoning"
    )
    ...
    assert "ok" in str(result).lower(), (
        f"The MCP ping tool did not return OK: {result!r}"
    )
