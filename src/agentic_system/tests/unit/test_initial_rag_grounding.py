...
def test_assignment_is_enriched_only_for_matching_task() -> None:
    executor = _executor_without_runtime()
    executor._initial_rag_context_by_task["task-1"] = {
        "source": "Qdrant RAG",
    ...
    grounded = executor._assignment_with_grounding(
        {"task_id": "task-1", "entity": "processing-service"}
    )
    untouched = executor._assignment_with_grounding(
        {"task_id": "task-2", "entity": "processing-service"}
    )
    ...
    assert "static_project_grounding" not in untouched
