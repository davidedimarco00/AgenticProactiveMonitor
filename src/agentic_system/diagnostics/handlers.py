...
class DiagnosticHandlers:
    ...
    async def inspect_container(self, target: str, parameters: dict[str, Any]) -> dict[str, Any]:
        def run():
    ...
    async def get_container_stats(self, target: str, parameters: dict[str, Any]) -> dict[str, Any]:
        def run():
            stats = self.docker.containers.get(target).stats(stream=False)
            cpu = stats.get('cpu_stats', {})
            memory = stats.get('memory_stats', {})
            return {'cpu_stats': cpu, 'memory_stats': memory, 'pids_stats': stats.get('pids_stats', {})}
        return await asyncio.to_thread(run)

    async def check_health_endpoint(self, target: str, parameters: dict[str, Any]) -> dict[str, Any]:
